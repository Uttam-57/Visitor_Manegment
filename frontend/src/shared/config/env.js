...
const normalizeApiOrigin = (baseUrl) => {
...
export const env = {
  API_BASE_URL: configuredApiBase || 'http://localhost:5000/api',
  API_ORIGIN: normalizeApiOrigin(configuredApiBase || 'http://localhost:5000/api'),
...
if (!configuredApiBase && env.IS_DEV) {
  logger.warn('VITE_API_BASE_URL is not configured. Falling back to http://localhost:5000/api')
}